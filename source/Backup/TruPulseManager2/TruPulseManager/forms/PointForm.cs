...
namespace TruPulseManager
{
    public partial class PointForm : Form
    {
        ...
        {
            int index = MainForm.MeasuredPoints.Count-1;

            nUpDownHeight.Value = Convert.ToDecimal(MainForm.MarkHeight);

            if (index == 0)
            {
                nUpDownID.Value = MainForm.StartPointID;
            }
            else
            {
                nUpDownID.Value = Convert.ToDecimal(MainForm.MeasuredPoints[index].ID + 1);
            }
        ...
        private void CalcCoordinates()
        {
            ...
            measuredPoint.StationID = Convert.ToInt32(MainForm.Station.ID);
            measuredPoint.CalcCoordinates(MainForm.Station);
        ...
        private void buttonStore_Click(object sender, EventArgs e)
        {
            ...
            MainForm.MarkHeight = Convert.ToDouble(nUpDownHeight.Value);
            
            //Ide kell egy újrarajzolás
            //MeasuredPoint.DrawingArea.ReDraw();

            if (MainForm.Profile == MainForm.Section.Add || MainForm.Profile == MainForm.Section.Insert)
            {
                MainForm.CrossSection.Indices.Add(measuredPoint.ID);
            }
