...
namespace TruPulseManager
{
    public partial class DrawingArea : UserControl
    {
        ...
        public void ReDraw()
        {
            ...
            ///
            //foreach (Vector item in MainForm.bspline)
            //{
            //    int sX = GetScreenX(item.X, ScreenX, Zoom);
            //    int sY = GetScreenY(item.Y, ScreenY, Zoom);
            //    controlDC.FillEllipse(Brushes.Yellow, sX-3, sY-3, 6, 6);
            //}

            for (int i = 0; i < MainForm.bspline.Count - 2; i += 1)
            {
                int sX1 = GetScreenX(MainForm.bspline[i].X, ScreenX, Zoom);
                int sY1 = GetScreenY(MainForm.bspline[i].Y, ScreenY, Zoom);
                int sX2 = GetScreenX(MainForm.bspline[i + 1].X, ScreenX, Zoom);
                int sY2 = GetScreenY(MainForm.bspline[i + 1].Y, ScreenY, Zoom);
                //int sX3 = GetScreenX(MainForm.bspline[i + 2].X, ScreenX, Zoom);
                //int sY3 = GetScreenY(MainForm.bspline[i + 2].Y, ScreenY, Zoom);
                //int sX4 = GetScreenX(MainForm.bspline[i + 3].X, ScreenX, Zoom);
                //int sY4 = GetScreenY(MainForm.bspline[i + 3].Y, ScreenY, Zoom);

                controlDC.DrawLine(Pens.Blue, sX1, sY1, sX2, sY2);

                //controlDC.DrawBezier(Pens.Blue, sX1, sY1, sX2, sY2, sX3, sY3, sX4, sY4);
            }

            ///

            /// DrawStation(Project.Station);
