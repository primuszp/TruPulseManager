...
namespace TruPulseManager
{
    public partial class MainForm : Form
    {
        ...
        public void CalcBSpline()
        {
            SharpMath myMath = new SharpMath();
            List<List<Vector>> shell = new List<List<Vector>>();
            Vector[] bspv = null;

            foreach (MeasuredPoint item in Project.MeasurePoints)
            {
                Vector v = new Vector(item.Coordinates.X, item.Coordinates.Y, item.Coordinates.Z);
                controll.Add(v);
            }

            bspv = myMath.cardinalSpline(controll, 0.5, false);

            controll.Clear();
            foreach (Vector item in bspv)
            {
                controll.Add(item);
            }

            bspline.Clear();
            myMath.ValidateKnots(controll.Count - 1);
            bspline = myMath.GetDeboor(controll, shell, 0, false);
        }
